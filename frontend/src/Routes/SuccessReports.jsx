...
export const SuccessReports = () => {
  ...
  function handleFilter(event){
    ...
    const newData = data.filter(row =>{
      debugger
      return  Object.values(row)[1].toLowerCase().includes(event.target.value.toLowerCase());
    })
  ...
  if(Loading) return <h1>Loading</h1>
    ...
    <>
      <Nav></Nav>
      <Header title = "Success Report"></Header>
      <div className="report">
        <div className="table-data">             
          <div className='text-end'><input type="text" placeholder='Search' onChange={handleFilter} /></div>
          <DataTable key={data} title={'Sent Reports'} columns={columns} data={filter ? filterData : data} 
          fixedHeader
          pagination
          responsive
          highlightOnHover
